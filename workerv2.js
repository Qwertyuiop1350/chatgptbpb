...
async function fetchWarpAccounts(env) {
  ...
  const fetchAccount = async (key) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Warp API returned status ${response.status}`);
      }
      const payload = await response.json();
      if (!payload?.config?.interface?.addresses?.v6 || !payload?.config?.client_id || !payload?.config?.peers?.[0]?.public_key) {
        throw new Error("Warp API response missing required config fields");
      }
      return payload;
    } catch (error) {
...
const CLUBHOUSE_UDP_PORTS = "443,3478,3479,5349";
...
function buildRoutingRules2(ctx, isWarp, isChain) {
  ...
  if (!isWarp) {
    ...
    addRoutingRule(rules, "reject", ["clubhouseapi.com", "clubhouse.pub", "agora.io", "agoraio.cn"], void 0, void 0, void 0, "udp", void 0, CLUBHOUSE_UDP_PORTS);
  }
...
function buildRoutingRules3(ctx, isChain, isBalancer, isWorkerless, isWarp) {
  ...
  if (!(isWarp || isWorkerless)) {
    ...
    addRoutingRule2(rules, void 0, ["domain:clubhouseapi.com", "domain:clubhouse.pub", "domain:agora.io", "domain:agoraio.cn"], void 0, CLUBHOUSE_UDP_PORTS, "udp", void 0, "block", false);
  }
...
var worker_default = {
  async fetch(request, env, context) {
    const runWithTimeout = () => withTimeout(this.handleFetch(request, env, context), 45000, "Request Timeout after 45s");
  ...
  async handleFetch(request, env, context) {
    try {
      ...
      // Clubhouse compatibility: force-disable QUIC/UDP:443 to avoid broken media sessions on worker-based tunnels
      if (ctx.settings.clubhouseCompatMode !== false) {
        ctx.settings.blockUDP443 = true;
      }
